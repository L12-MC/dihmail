...
from mail import new_alias_for_user, send_message, get_inbox, get_message_detail, decrypt_message_id
...
def register():
    if request.method == "POST":
        ...
        try:
            ...
            flash(f"User {local} registered successfully! You can now login with the password you just created.", "success")
            return redirect(url_for("login"))
...
def login():
    if request.method == "POST":
        ...
        if not user:
            ...
            flash("Invalid password. Please try again or register a new account.", "error")
    return render_template("login.html")
...
def send():
    ...
    if request.method == "POST":
        ...
        try:
            result = send_message(user_local, recipient, message)
